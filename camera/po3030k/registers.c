...
#define MCLK				((long) clock_get_cycle_frequency()/2)  /* 20Mhz */
#define MCLK_P_MS			((1000./MCLK))  /* Master clock period in ms */
...
#define DEVICE_ID  			PO3030K_DEVICE_ID
 ...
 * \sa po3030k_read_cam_registers 
 */
void po3030k_write_cam_registers(void) {
	int i;
	for(i=0;i < 2*NB_REGISTERS; i+=2 ) 
			i2c_write(I2C_1,DEVICE_ID,cam_reg[i],&(cam_reg[i+1]), 1);
...
void po3030k_read_cam_registers(void) {
	int i;
	for(i=0;i < 2*NB_REGISTERS; i+=2) 
		i2c_read(I2C_1,DEVICE_ID,cam_reg[i],&(cam_reg[i+1]), 1);

}

/*! Set the camera color mode 
 * \warning This is an internal function, use po3030k_config_cam
 * \param mode The color mode
 * \return Zero if OK, non-zero if an error occur
 * \sa Datasheet p. 31, po3030k_write_cam_registers and po3030k_config_cam
 */
 ...
 * \return Zero if OK, non-zero if an error occur
 * \sa Datasheet p. 28 and po3030k_config_cam
 */
 ...
 * \sa Datasheet p. 26 and po3030k_config_cam
 */
 ...
 * \warning This is an internal function, use po3030k_ConfigCam
 * \param start The start column
 * \param stop The stop column
 * \return Zero if OK, non-zero if an error occur
 * \sa Datasheet p.20-21, po3030k_write_cam_registers, po3030k_set_wy and po3030k_config_cam
 */
 ...
 * \return Zero if OK, non-zero if an error occur
 * \sa Datasheet p.20-21, po3030k_WriteCamRegisters, po3030k_SetWX and po3030k_ConfigCam
 */
 ...
 * \return Zero if OK, non-zero if an error occur
 * \sa Datasheet p.42-43, po3030k_write_cam_registers and po3030k_config_cam
 */
 ...
 */
int  po3030k_set_color_gain(unsigned char global, unsigned char red,
 ...
 * - 0b00 => Red
 * - else => Blue
 * \sa Datasheet p. 38, 50, 57-58 and po3030k_WriteGammaCoef
 */
 ...
 * \sa po3030k_set_gamma_coef
 */

void po3030k_write_gamma_coef(void) {
	i2c_write(I2C_1, DEVICE_ID,cam_reg[GAMMASELCOL_BASE - 1],&(cam_reg[GAMMASELCOL_BASE]), 1);
 ...
 * and then write all registers with po3030k_WriteCamRegisters
...
void po3030k_SetColorMatrix(unsigned char array[3*3]) {
	int i;
	for(i = 0; i< sizeof(array); i++) 
 ...
 * \param cb Cb tone
 * \param cr Cr tone
 * \sa po3030k_set_sepia and Datasheet p. 41 and 74
 */
...
void po3030k_set_max_min_exp(unsigned int max, unsigned int min) {
 ...
 */
void po3030k_set_max_min_awb(unsigned char minb, unsigned char maxb, unsigned char minr,
 ...
 * \sa Datasheet p.29 po3030k_set_flicker_detection() po3030k_set_flicker_man_set()
 */
 ...
 * flicker.
 * \sa Datasheet p.29 po3030k_set_flicker_mode() po3030k_set_flicker_man_set()
 */
 ...
 * \sa Datasheet p.29-30 po3030k_set_flicker_detection() po3030k_set_flicker_mode()
 */
